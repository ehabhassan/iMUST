...
INTERFACE iSMOOTH
 MODULE PROCEDURE iSMOOTH2D,iSMOOTH3D
END INTERFACE iSMOOTH

INTERFACE PYSMOOTH
 MODULE PROCEDURE PYSMOOTH2D
END INTERFACE PYSMOOTH

INTERFACE QSHEPFIT
 MODULE PROCEDURE QSHEPFIT2D,QSHEPFIT3D
END INTERFACE QSHEPFIT

INTERFACE AVG
 MODULE PROCEDURE R8AVG,R4AVG,I8AVG,I4AVG
END INTERFACE AVG

INTERFACE STDV
 MODULE PROCEDURE R8STDV,R4STDV,I8STDV,I4STDV
END INTERFACE STDV

CONTAINS

!*************************************************
! iSMOOTH SUNROUTINE INTERFACE FOR INTERPOLATION
!*************************************************

      SUBROUTINE iSMOOTH2D(iTYPE,iRANK,inX2D,inY2D,inZ2D,X2D,Y2D,Z2D,SDIM) 
       IMPLICIT NONE 
       CHARACTER(*)                        :: iTYPE,iRANK 
       INTEGER(4),OPTIONAL                 :: SDIM 
       INTEGER(4)                          :: mx,my,mz 
       INTEGER(4)                          :: exmx,exmy,exmz 
       INTEGER(4)                          :: iDIMS(2),DIMS(2)
       INTEGER(4)                          :: N ,ferror
       REAL(4), INTENT(IN), DIMENSION(:,:) :: inX2D,inY2D,inZ2D 
       REAL(4), INTENT(IN), DIMENSION(:,:) ::   X2D,  Y2D 
       REAL(4), INTENT(OUT),DIMENSION(:,:) ::   Z2D 
       REAL(4), ALLOCATABLE,DIMENSION(:,:) ::iDATAMAT,DATAMAT 
                                                                        

       IF(TRIM(iTYPE).EQ."qshep") THEN
        CALL QSHEPFIT(inX2D,inY2D,inZ2D,X2D,Y2D,Z2D)
       ELSE IF(TRIM(iTYPE).EQ."python") THEN
        iDIMS=SHAPE(inX2D);DIMS=SHAPE(X2D)
        OPEN(UNIT=102,FILE="pyInterpXXXX.dat",STATUS='NEW',ACTION='WRITE',ACCESS="APPEND",IOSTAT=ferror)
        IF(ferror.NE.0) OPEN(UNIT=102,FILE="pyInterpXXXX.dat",STATUS='REPLACE',ACTION='WRITE',IOSTAT=ferror)
        DO mx=1,iDIMS(1)
         DO my=1,iDIMS(2)
          WRITE(102,"(1ES15.8,T23,1ES15.8,T46,1ES15.8)") inX2D(mx,my),inY2D(mx,my),inZ2D(mx,my)
         END DO
        END DO
        CLOSE(102)
        CALL PYSMOOTH(iRANK,"pyInterpXXXX.dat",iDIMS,DIMS,(/1,4/),X2D,Y2D,Z2D)
       ELSE
        iDIMS=SHAPE(inZ2D) 
        mx=iDIMS(1) 
        my=iDIMS(2) 
        ...
        IF(ALLOCATED(iDATAMAT)) DEALLOCATE(iDATAMAT) 
        IF(ALLOCATED( DATAMAT)) DEALLOCATE( DATAMAT) 
        ALLOCATE(iDATAMAT(  mz,3)) 
        ALLOCATE( DATAMAT(exmz,3)) 
                                                                         
        iDATAMAT(:,1)=RESHAPE(inX2D,(/  mz/)) 
        iDATAMAT(:,2)=RESHAPE(inY2D,(/  mz/)) 
        iDATAMAT(:,3)=RESHAPE(TRANSPOSE(inZ2D),(/  mz/)) 
         DATAMAT(:,1)=RESHAPE(  X2D,(/exmz/)) 
         DATAMAT(:,2)=RESHAPE(  Y2D,(/exmz/)) 
                                                                        
        IF(PRESENT(SDIM)) THEN 
         CALL FITPACK(iTYPE,iRANK,SDIM,iDIMS,DIMS,iDATAMAT,DATAMAT) 
        ELSE 
         CALL FITPACK(iTYPE,iRANK,1,iDIMS,DIMS,iDATAMAT,DATAMAT) 
        END IF 
                                                                        
        Z2D=TRANSPOSE(RESHAPE(DATAMAT(:,3),(/exmy,exmx/))) 
       END IF
   
       RETURN 
      END SUBROUTINE iSMOOTH2D

      SUBROUTINE iSMOOTH3D(iTYPE,iRANK,inX3D,inY3D,inZ3D,inF3D,X3D,Y3D,Z3D,F3D,SDIM) 
       IMPLICIT NONE 
       CHARACTER(*)                          :: iTYPE,iRANK 
       INTEGER(4),OPTIONAL                   :: SDIM 
       INTEGER(4)                            :: mx,my,mz 
       INTEGER(4)                            :: exmx,exmy,exmz 
       INTEGER(4)                            :: iDIMS(3),DIMS(3) 
       REAL(4), INTENT(IN), DIMENSION(:,:,:) :: inX3D,inY3D,inZ3D,inF3D
       REAL(4), INTENT(IN), DIMENSION(:,:,:) ::   X3D,  Y3D,  Z3D 
       REAL(4), INTENT(OUT),DIMENSION(:,:,:) ::   F3D 
       REAL(4), ALLOCATABLE,DIMENSION(:,:)   ::iDATAMAT,DATAMAT 
                                                                        
                                                   
       IF(TRIM(iTYPE).EQ."qshep") THEN
        CALL QSHEPFIT(inX3D,inY3D,inZ3D,inF3D,X3D,Y3D,Z3D,F3D)
       END IF
                                                                     
       RETURN 
      END SUBROUTINE iSMOOTH3D



!*******************************************************
!   INTERPOLATION SUBROUTINE INTERFACE FOR FITPACK
!*******************************************************

      SUBROUTINE FITPACK(iTYPE,iRANK,SDIMS,iDIMS,DIMS,iDMAT,DMAT) 
       IMPLICIT NONE 
!      INPUT/OUTPUT VARIABLES                                           
       REAL(4),    INTENT(IN),   DIMENSION(:,:) :: iDMAT 
       REAL(4),    INTENT(INOUT),DIMENSION(:,:) ::  DMAT 
       INTEGER(4), INTENT(IN),   DIMENSION(:)   :: iDIMS,DIMS 
       INTEGER(4)                               :: SDIMS,iPAR(2)=(/0,0/) 
       CHARACTER(*)                             :: iTYPE,iRANK 
!      LOCAL VARAIBLES                                                  
       INTEGER(4)              :: mx,my,mz 
       INTEGER(4)              :: exmx,exmy,exmz 
       INTEGER(4)              :: xdense,ydense 
       INTEGER(4)              :: nx,ny 
       INTEGER(4)              :: kx,ky 
       INTEGER(4)              :: nxest,nyest 
       INTEGER(4)              :: cdim 
       INTEGER(4)              :: nmax 
       INTEGER(4)              :: bx,by,b1,b2 
       INTEGER(4)              :: iopt,ier 
       INTEGER(4)              :: kwrk,lwrk,mwrk 
       INTEGER(4)              :: iD1,iD2,MATDIMS(2) 
       INTEGER(4), ALLOCATABLE :: iwrk(:),iwk(:) 
       REAL(4),    ALLOCATABLE :: wrk(:),wk(:) 
       REAL(4),    ALLOCATABLE :: tx(:),ty(:) 
       REAL(4),    ALLOCATABLE :: spco(:) 
       REAL(4),    ALLOCATABLE :: wt(:) 
       REAL(4),    ALLOCATABLE :: X1D(:),Y1D(:),Z1D(:) 
       REAL(4)                 :: xb,xe,yb,ye 
       REAL(4)                 :: fp,s 
       REAL(4)                 :: eps=1.0e-6 
                                                                        
       IF(SIZE(iDIMS).EQ.2) THEN 
        mx=iDIMS(1) 
        my=iDIMS(2) 
        mz=iDIMS(1)*iDIMS(2) 
        exmx=DIMS(1) 
        ...
        IF(TRIM(iRANK).EQ."quadratic")THEN 
         kx=2 
         ky=2 
        ELSE IF(TRIM(iRANK).EQ."cubic") THEN 
         kx=3 
         ky=3 
        ELSE IF(TRIM(iRANK).EQ."quartic") THEN 
         kx=4 
         ky=4 
        ELSE IF(TRIM(iRANK).EQ."quintic") THEN 
         kx=5 
         ky=5 
        END IF 
...
!        nxest=INT(SQRT(mz/2.0))+kx+1                                   
!        nyest=INT(SQRT(mz/2.0))+ky+1                                   
         nxest=2*(kx+1) 
         nyest=2*(ky+1) 
         nx=min(nxest,mz+kx+1) 
         ny=min(nyest,mz+ky+1) 
         ...
         cdim=(nxest-kx-1)*(nyest-ky-1) 
         bx=kx*(nxest-kx-1)+(ky-1) 
         by=ky*(nyest-ky-1)+(kx-1) 
         IF(bx.le.by) THEN 
          b1=bx 
          b2=b1+(nxest+kx+1)-ky 
         ELSE 
          ...
          b2=b1+(nyest-ky-1)-kx 
         END IF 
         kwrk=mz+(nxest-2*kx-1)*(nyest-2*ky-1) 
         lwrk=(nxest-kx-1)*(nyest-ky-1)*(2+b1+b2)                       &
     &       +2*((nxest-kx-1)+(nyest-ky-1)+                             &
     &        (MAX(kx,ky)+1)*(mz+MAX(nxest,nyest))+                     &
     &         MAX(nxest,nyest)-kx-ky)                                  &
     &       +b2+1                                                      
         mwrk=(nxest-kx-1)*(nyest-ky-1)*(b2+1)+b2 
                                                                        
         IF(ALLOCATED(wrk)) DEALLOCATE(wrk) 
         IF(ALLOCATED(wk))  DEALLOCATE(wk) 
         IF(ALLOCATED(iwrk)) DEALLOCATE(iwrk) 
         IF(ALLOCATED(spco)) DEALLOCATE(spco) 
         ALLOCATE(iwrk(kwrk)) 
         ALLOCATE(wrk(lwrk)) 
         ALLOCATE(wk(mwrk)) 
         ALLOCATE(spco(cdim)) 
         ...
         s=REAL(mz) 
         call surfit(iopt,mz,x1d,y1d,z1d,wt,xb,xe,yb,ye                 &
     &        ,kx,ky,s,nxest,nyest,nmax,eps                             &
     &        ,nx,tx,ny,ty,spco,fp,wrk,lwrk,wk,mwrk,iwrk,kwrk,ier)      
         IF(ier.GT.0) print*, "Error in surfit subroutine. Error: ", ier 
         IF(ier.EQ.1) print*, "s = ", s, ", fp = ", fp 
         ...
         ALLOCATE(Z1D(exmz)) 
         X1D=DMAT(1:exmx,1) 
         Y1D=(/(DMAT(iD2,2),iD2=1,exmz,exmx)/) 
                                                                        
         lwrk=exmx*(kx+1)+exmy*(ky+1) 
         kwrk=exmx+exmy 
         ...
         call bispev(tx,nx,ty,ny,spco,kx,ky                             &
     &              ,x1d,exmx,y1d,exmy,z1d                              &
     &              ,wrk,lwrk,iwrk,kwrk,ier)                            
         IF(ier.GT.0) print*, "Error in bispev subroutine. Error: ", ier 
!****************************************************************       
!                         RECTANGLE                                     
!****************************************************************       
        ELSE IF(TRIM(iTYPE).EQ."rectangle") THEN 
         IF(ALLOCATED(X1D)) DEALLOCATE(X1D) 
         IF(ALLOCATED(Y1D)) DEALLOCATE(Y1D) 
         IF(ALLOCATED(Z1D)) DEALLOCATE(Z1D) 
         ALLOCATE(X1D(mx)) 
         ...
         yb=MINVAL(Y1D) 
         ye=MAXVAL(Y1D) 
         if(any(x1d.lt.xb)) print*, "Error in xb" 
         if(any(x1d.gt.xe)) print*, "Error in xe" 
         if(any(y1d.lt.yb)) print*, "Error in yb" 
         if(any(y1d.gt.ye)) print*, "Error in ye" 
                                                                        
         iopt=0 
         s=0.0 
                                                                        
         IF((iopt.GE.0).AND.(s.GE.0.0)) THEN 
          nxest=mx+kx+1 
          nyest=my+kx+1 
         ELSE 
          nxest=2*(kx+1) 
          nyest=2*(kx+1) 
         END IF 
         nx=min(nxest,mx+kx+1) 
         ny=min(nyest,my+ky+1) 
         ...
         lwrk=4+nxest*(my+2*kx+5)+nyest*(mx+2*ky+5)                     &
     &       +mx*(kx+1)+my*(ky+1)+MAX(my,nxest)                         
         ...
         call regrid(iopt,mx,x1d,my,y1d,z1d,xb,xe,yb,ye,kx,ky,s,        &
     &        nxest,nyest,nx,tx,ny,ty,spco,fp,wrk,lwrk,iwrk,kwrk,ier)   
         IF(ier.GT.0) print*, "Error in regrid subroutine. Error: ", ier 
         ...
         call bispev(tx,nx,ty,ny,spco,kx,ky,x1d,exmx,y1d,exmy,z1d,      &
     &             wrk,lwrk,iwrk,kwrk,ier)                              
         IF(ier.GT.0) print*, "Error in bispev subroutine. Error: ", ier 
...
!                         PARAMETRIC                                    
!****************************************************************       
        ELSE IF(TRIM(iTYPE).EQ."parametric") THEN 
         IF(ALLOCATED(X1D)) DEALLOCATE(X1D) 
         IF(ALLOCATED(Y1D)) DEALLOCATE(Y1D) 
         IF(ALLOCATED(Z1D)) DEALLOCATE(Z1D) 
         ALLOCATE(X1D(mx)) 
         ...
         IF((iopt.GE.0).AND.(s.EQ.0.0)) THEN 
          nxest=mx+kx+1+2*iPAR(1) 
          nyest=my+kx+1+2*iPAR(2) 
         ELSE 
          nxest=2*(kx+1+iPAR(1)) 
          nyest=2*(kx+1+iPAR(2)) 
         END IF 
         nx=min(nxest,mx+kx+1+2*iPAR(1)) 
         ny=min(nyest,my+ky+1+2*iPAR(2)) 
         ...
         cdim=(nxest-kx-1)*(nyest-ky-1)*SDIMS 
         kwrk=3+mx+my+nxest+nyest 
         lwrk=4+nxest*(my*SDIMS+2*kx+5+4*iPAR(1))                       &
     &       +nyest*(mx*SDIMS+2*ky+5+4*iPAR(2))                         &
     &       +mx*(kx+1)+my*(ky+1)                                       &
     &       +SDIMS*MAX(my,nxest)                                       
         ...
         call parsur(iopt,ipar,sdims,mx,x1d,my,y1d,z1d,s,nxest,nyest,   &
     &        nx,tx,ny,ty,spco,fp,wrk,lwrk,iwrk,kwrk,ier)               
         IF(ier.GT.0) print*, "Error in parsur subroutine. Error: ", ier 
         IF(ier.EQ.1) print*, "s = ", s, ", fp = ", fp 
         ...
         call surev(sdims,tx,nx,ty,ny,spco                              &
     &             ,x1d,exmx,y1d,exmy,z1d,exmz*sdims                    &
     &             ,wrk,lwrk,iwrk,kwrk,ier)                             
         IF(ier.GT.0) print*, "Error in surev subroutine. Error: ", ier 
       ...
       RETURN 
      END SUBROUTINE FITPACK

!*********************************************
!           QSHEP 2D INTERPOLATION
!*********************************************

SUBROUTINE QSHEPFIT2D(AX2D,AY2D,AF2D,BX2D,BY2D,BF2D,ONR)
 IMPLICIT NONE
 REAL(4),                 DIMENSION(:,:) :: AX2D,AY2D,AF2D,BX2D,BY2D,BF2D
 REAL(4),    ALLOCATABLE, DIMENSION(:)   :: X1D,Y1D,F1D
 REAL(4),    ALLOCATABLE                 :: RSQ(:),A(:,:)
 REAL(4)                                 :: PX,PY,XMIN,YMIN,DX,DY,RMAX
 REAL(4), OPTIONAL                       :: ONR ! PRECENTAGE OF THE DATA YOU WANT TO USE IN INTERPOLATION
 INTEGER(4), ALLOCATABLE                 :: LCELL(:,:),LNEXT(:)
 INTEGER(4),              DIMENSION(2)   :: MATDIMS
 INTEGER(4)                              :: iD1,iD2
 INTEGER(4)                              :: N,NQ,NW,NR,IER

!PRINT*, "QSHEP2D INTERPOLATION >>> STARTED"

 MATDIMS=SHAPE(AX2D)
 N=MATDIMS(1)*MATDIMS(2)
 ...
 CALL QSHEP2(N,X1D,Y1D,F1D,NQ,NW,NR,LCELL,LNEXT,XMIN,YMIN,DX,DY,RMAX,RSQ,A,IER)
 IF(IER.NE.0) PRINT*, "QSHEP2 ECOUNTERED AN ERROR = ",IER
 ...
 DO iD1=1,MATDIMS(1)
  DO iD2=1,MATDIMS(2)
   PX=BX2D(iD1,iD2)
   PY=BY2D(iD1,iD2)
   BF2D(iD1,iD2)=QS2VAL(PX,PY,N,X1D,Y1D,F1D,NR,LCELL,LNEXT,XMIN,YMIN,DX,DY,RMAX,RSQ,A)
  END DO
 END DO

!PRINT*, "QSHEP2D INTERPOLATION >>> FINISHED"
 RETURN
END SUBROUTINE QSHEPFIT2D

!*********************************************
!           QSHEP 3D INTERPOLATION
!*********************************************

SUBROUTINE QSHEPFIT3D(AX3D,AY3D,AZ3D,AF3D,BX3D,BY3D,BZ3D,BF3D,ONR)
 IMPLICIT NONE
 REAL(4),                 DIMENSION(:,:,:) :: AX3D,AY3D,AZ3D,AF3D,BX3D,BY3D,BZ3D,BF3D
 REAL(4),    ALLOCATABLE, DIMENSION(:)     :: X1D,Y1D,Z1D,F1D
 REAL(4),    ALLOCATABLE                   :: RSQ(:),A(:,:)
 REAL(4)                                   :: PX,PY,PZ
 REAL(4)                                   :: XYZMIN(3),XYZDEL(3),RMAX
 REAL(4),    OPTIONAL                      :: ONR ! PRECENTAGE OF THE DATA YOU WANT TO USE IN INTERPOLATION
 INTEGER(4), ALLOCATABLE                   :: LCELL(:,:,:),LNEXT(:)
 INTEGER(4),              DIMENSION(3)     :: MATDIMS
 INTEGER(4)                                :: iD1,iD2,iD3
 INTEGER(4)                                :: N,NQ,NW,NR,IER

!PRINT*, "QSHEP3D INTERPOLATION >>> STARTED"

 MATDIMS=SHAPE(AX3D)
 N=MATDIMS(1)*MATDIMS(2)*MATDIMS(3)
 ...
 CALL QSHEP3(N,X1D,Y1D,Z1D,F1D,NQ,NW,NR,LCELL,LNEXT,XYZMIN,XYZDEL,RMAX,RSQ,A,IER)
 IF(IER.NE.0) PRINT*, "QSHEP3 ECOUNTERED AN ERROR = ",IER
 ...
 DO iD1=1,MATDIMS(1)
  DO iD2=1,MATDIMS(2)
   DO iD3=1,MATDIMS(3)
    ...
    PY=BY3D(iD1,iD2,iD3)
    PZ=BZ3D(iD1,iD2,iD3)
    BF3D(iD1,iD2,iD3)=QS3VAL(PX,PY,PZ,N,X1D,Y1D,Z1D,F1D,NR,LCELL,LNEXT,XYZMIN,XYZDEL,RMAX,RSQ,A)
   END DO
  END DO
 END DO

!PRINT*, "QSHEP3D INTERPOLATION >>> FINISHED"
 RETURN
END SUBROUTINE QSHEPFIT3D



!*************************************
! QSHEP2D INTERPOLATION SUBROUTINES
!*************************************
...
!  Parameters:
!
!    Input, real PX, PY, the coordinates of the point at which the
...
!    defining the interpolant Q.  Computed by QSHEP2.
!
!    Output, real Q, QX, QY, the value of the interpolant, and its derivatives
...
5   continue
    ...
    do j = 1, irow-1
      jp1 = j + 1
      call givens2d ( b(j,j), b(j,irow), c, s )
      call rotate2d ( 6-j, c, s, b(jp1,j), b(jp1,irow) )
    end do
...
10  continue
    ...
    do i = 1, 3
      ...
      do j = i, 5
        jp1 = j + 1
        call givens2d ( b(j,j), b(j,6), c, s )
        call rotate2d ( 6-j, c, s, b(jp1,j), b(jp1,6) )
      end do
...
!*************************************
! QSHEP3D INTERPOLATION SUBROUTINES
!*************************************
...
5   continue
    ...
    do j = 1, irow-1
      jp1 = j + 1
      call givens3d (b(j,j),b(j,irow),c,s)
      call rotate3d (10-j,c,s,b(jp1,j),b(jp1,irow))
    end do
...
10  continue
    ...
    do i = 1, 6
      ...
      do j = i, 9
        jp1 = j + 1
        call givens3d (b(j,j),b(j,10),c,s)
        call rotate3d (10-j,c,s,b(jp1,j),b(jp1,10))
      end do
...
2 continue
  ...
  return
end subroutine givens3d
subroutine rotate3d ( n, c, s, x, y )
!
!***********************************************************************
!
...
!**********************************************
!  TIEGCM HIEGHT INTERPOLATION SUBROUTINES
!**********************************************
!
!
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!
! Routines to do linear or log interpolation of gcm fields to
! a linear height scale. Dimensions of fin, fht, and fout vary
! depending on routine called.
!
! glbhtint() is global height interpolator:
!   fin and fht (nlon,nzp,nlat) = global input and gcm heights fields
!   fout(nlon,nlat,nhts) = global interpolated field returned
!
! glbhtin() is same as glbhtint except that the intput fields fin and fht
!   are dimensioned (nlon,nlat,nzp) rather than (nlon,nzp,nlat). This is
!   more convenient for some codes, e.g. "super_procs" gcm8proc, gcm9proc,
!   etc.
!
! cuthtint() is height interpolator at some slice of the grid (e.g.,
!   along a latitude (idim1 = imx) or longitude (idim1 = jmx)
!   fin and fht (idim1,nzp) = input and gcm heights fields (at some cut)
!   fout(idim1,nhts) = interpolated field returned
!
! intloc() interpolates fields at some location (lat,lon gcm grid point)
!
! Inputs common to all routines (dimensions vary):
!
!   fin: the gcm field at constant pressures
!   fht: gcm heights at constant pressures
!   hts: the height scale at which to do the interpolation
!   nhts: number of heights in hts
!   logint: if logint > 0, do log (exponential) interpolation,
!           otherwise linear
!   spval: if != 0., is special value returned in fout if
!          ht(i) > max gcm height, or ht(i) < min gcm height.
!          if = 0., extrapolation will be attempted if ht(i) out of range
!   iprnt: print stuff out if > 0 (e.g., if spval is used)
!
! Outputs common to all routines (dimension of fout varies):
!
!   fout: returned field, interpolated to desired height scale
!   ier: non-zero if an error has occurred
!
! Required routines:
!   fminmax(), bracket()
!
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!
      SUBROUTINE GLBHTIN(Fin,Fht,Nlon,Nlat,Nzp,Fout,Hts,Nhts,Logint,Spval,Ier,Iprnt)
      IMPLICIT NONE
!*--GLBHTIN137
!*** Start of declarations inserted by SPAG
      REAL col , exparg , f1 , Fht , Fin , Fout , htmax , htmin , Hts , Spval
      INTEGER i , Ier , ierr , Iprnt , j , k , k0 , k1 , Logint , Nhts , Nlat , Nlon , Nzp
!*** End of declarations inserted by SPAG
!
! 2/2/93: Same as glbhtint, except that fin and fht are dimensioned
!         (imx,jmx,kmx) rather than (imx,kmx,jmx). This is preferred
!         by "super-procs" (i.e., gcm8proc, gcm9proc, etc)
!
! Do linear or log interpolation of fin to constant height surfaces
!   in hts(nhts). Interpolated field returned in fout.
!
! On input:
!   fin(nlon,nlat,nzp) = global input field
!   fht(nlon,nlat,nzp) = global heights field
!   nlon,nlat,nzp = dimensions of fin and fht
!   hts(nhts) = heights at which to interpolate
!   logint <= 0 -> do linear interpolation,
!                  otherwise do exponential (log) interpolation
!   spval: If spval is non-zero and hts(k) is not within fht range
!          at the current grid point, then fout gets spval at the
!          current grid point.
!          If spval=0 and hts(k) is out of range, then
!          fout will be extrapolated to hts(k)
!   iprnt = 1 -> some error msgs will be printed if necessary
!
! On output:
!   fout(nlon,nlat,nhts) = contains interpolated field
!   ier = non-zero if an error has occurred
!
      DIMENSION Fin(Nlon,Nlat,Nzp) , Fht(Nlon,Nlat,Nzp) ,               &
...
!
      DO j = 1 , Nlat
         DO i = 1 , Nlon
            ...
            DO k = 1 , Nhts
               IF ( Hts(k).GE.htmin .AND. Hts(k).LE.htmax ) THEN
                  CALL BRACKET(Hts(k),col,Nzp,1,k0,k1,ierr)
                  IF ( ierr.NE.0 ) THEN
                     IF ( Iprnt.GT.0 ) WRITE (6,                        &
     &"('glbhtin: error from',                    ' bracket: i j=',2i3,'&
     & k=',i3,' hts(k)=',f8.2,                    ' hts col=',/(6f10.3))&
     &") i , j , k , Hts(k) , col
                     Fout(i,j,k) = Spval
                     GOTO 20
                  ENDIF
               ELSEIF ( Spval.NE.0. ) THEN
                  Fout(i,j,k) = Spval
                  GOTO 20
               ELSE
                  k0 = Nzp - 1
               ...
               ENDIF
!
! Do linear or log interpolation:
!
               IF ( Logint.LE.0 ) THEN
                  f1 = (Hts(k)-col(k0))/(col(k1)-col(k0))
                  Fout(i,j,k) = f1*Fin(i,j,k1) + (1.-f1)*Fin(i,j,k0)
               ELSEIF ( Fin(i,j,k1).GT.0. .AND. Fin(i,j,k1)             &
                      & .NE.Spval .AND. Fin(i,j,k0).GT.0. .AND.         &
                      & Fin(i,j,k0).NE.Spval ) THEN
                  exparg = (ALOG(Fin(i,j,k1)/Fin(i,j,k0))               &
                         & /(col(k1)-col(k0)))*(Hts(k)-col(k0))
                  Fout(i,j,k) = Fin(i,j,k0)*EXP(exparg)
               ELSE
                  Fout(i,j,k) = Spval
               ENDIF
 20         ENDDO
...
!*==BRACKET.spg  processed by SPAG 6.70Dc at 13:44 on 12 May 2013
...
!******************************************                                                                        
!   FIRTPACK INTERPOLATION SUBROUTINES                     
!******************************************                             
...
!  apply that transformation to the rows of (a).                        
 ...
 SUBROUTINE PYSMOOTH2D(INTERPTYPE,FNAME,OLDDIMS,NEWDIMS,DFC,X2D,Y2D,Z2D)
  IMPLICIT NONE
  INTEGER(4)          :: NDIMS,OLDDIMS(:),NEWDIMS(:),DFC(:)
  INTEGER(4)          :: FNL,ICONTR,ferror
  LOGICAL             :: iflag=.false.
  CHARACTER(*)        :: INTERPTYPE,FNAME
  CHARACTER*70        :: TEMPTEXT
  REAL(4), ALLOCATABLE:: X1D(:),Y1D(:),Z1D(:)
  REAL(4)             :: X2D(:,:),Y2D(:,:),Z2D(:,:)
  REAL(4)             :: X
  ...
  NDIMS=3
  FNL=LEN_TRIM(FNAME)
  CALL INTERPDOTPY(INTERPTYPE,FNAME,NDIMS,OLDDIMS,NEWDIMS,DFC)
  FNL=LEN_TRIM(FNAME)
  TEMPTEXT="python ex"//fName(1:FNL-8)//".py"
  CALL SYSTEM(TRIM(TEMPTEXT))
  ALLOCATE(X1D(NEWDIMS(1)*NEWDIMS(2)))
  ALLOCATE(Y1D(NEWDIMS(1)*NEWDIMS(2)))
  ALLOCATE(Z1D(NEWDIMS(1)*NEWDIMS(2)))
 !TEMPTEXT="./ex"//TRIM(FNAME)
  TEMPTEXT="ex"//TRIM(FNAME)
  OPEN(UNIT=102,FILE=TRIM(TEMPTEXT),STATUS='OLD',ACTION='READWRITE',ACCESS='SEQUENTIAL',IOSTAT=ferror)
  DO ICONTR=1,NEWDIMS(1)*NEWDIMS(2)
   READ(UNIT=102,FMT='(1F12.9)') X1D(ICONTR)
   READ(UNIT=102,FMT='(1F12.9)') Y1D(ICONTR)
   READ(UNIT=102,FMT='(1F12.9)') Z1D(ICONTR)
  END DO
  CLOSE(UNIT=102)
  X2D=RESHAPE(X1D,(/NEWDIMS(1),NEWDIMS(2)/))
  Y2D=RESHAPE(Y1D,(/NEWDIMS(1),NEWDIMS(2)/))
  Z2D=RESHAPE(Z1D,(/NEWDIMS(1),NEWDIMS(2)/))
  IF(ALLOCATED(X1D)) DEALLOCATE(X1D)
  IF(ALLOCATED(Y1D)) DEALLOCATE(Y1D)
  IF(ALLOCATED(Z1D)) DEALLOCATE(Z1D)
  TEMPTEXT="rm ex"//fName(1:FNL-8)//".py"
  CALL SYSTEM(TRIM(TEMPTEXT))
  TEMPTEXT="rm ex"//fName(1:FNL)
  CALL SYSTEM(TRIM(TEMPTEXT))

  RETURN
 END SUBROUTINE PYSMOOTH2D


 SUBROUTINE INTERPDOTPY(interpType,oldName,NDIMS,OLDDIMS,NEWDIMS,DFC,newName)
  IMPLICIT NONE

  INTEGER, DIMENSION(:), INTENT(IN) :: DFC
  INTEGER, INTENT(IN) :: OLDDIMS(2),NEWDIMS(2),NDIMS
  INTEGER :: UFOLDDIMS(2)
  INTEGER :: N1
  INTEGER :: DFtype,Dtype
  INTEGER :: ferror,FNL,I
  CHARACTER(*) :: interpType,oldName
  CHARACTER(*), OPTIONAL :: newName
  CHARACTER*200 :: TEMPTEXT
  CHARACTER*50 :: TEMPTEXT1,TEMPTEXT2
  CHARACTER*75 :: TEMPTEXT3,TEMPTEXT4
  CHARACTER*100 :: TEMPTEXT5
  CHARACTER*10 :: vMIN,vMAX,vAVG,txDIM1,txDIM2,txDIM3
  LOGICAL :: IFLAG=.FALSE.

  DFtype=DFC(1);Dtype=DFC(2)

  N1=OLDDIMS(1)

  IF(DFtype.EQ.2) THEN
   IF(NDIMS.EQ.1) THEN
    UFOLDDIMS(1)=OLDDIMS(1)*OLDDIMS(2)
    UFOLDDIMS(2)=1
...
! Creating/Initializing/Adding the Data File to the mainPy.py:
  FNL=LEN_TRIM(oldName)
  TEMPTEXT="./ex"//oldName(1:FNL-8)//".py"
...
! IF(ferror/=0) THEN
   ...
   WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
   ...
   WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
   TEMPTEXT='from scipy import interpolate'
   WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
   ...
   WRITE(101,*) TRIM(TEMPTEXT)
   FNL=LEN_TRIM(oldName)
   TEMPTEXT="print 'INTERPOLATION FILE: ex"//oldName(1:FNL-8)//".py IS IN PROCESSING ...'"
   WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
   TEMPTEXT=""
   WRITE(101,*) TRIM(TEMPTEXT)
! END IF
  ...
  IF(DFtype.EQ.2) THEN
   IF((UFOLDDIMS(1)/1000000000).GE.1) THEN
    WRITE(vMAX,'(I10)') N1
   ELSEIF((UFOLDDIMS(1)/100000000).GE.1) THEN
    WRITE(vMAX,'(I9)') N1
   ELSEIF((UFOLDDIMS(1)/10000000).GE.1) THEN
    WRITE(vMin,'(I8)') UFOLDDIMS(1)
   ELSEIF((UFOLDDIMS(1)/1000000).GE.1) THEN
    WRITE(vMin,'(I7)') UFOLDDIMS(1)
   ELSEIF((UFOLDDIMS(1)/100000).GE.1) THEN
    WRITE(vMin,'(I6)') UFOLDDIMS(1)
   ELSEIF((UFOLDDIMS(1)/10000).GE.1) THEN
    WRITE(vMin,'(I5)') UFOLDDIMS(1)
   ELSEIF((UFOLDDIMS(1)/1000).GE.1) THEN
    WRITE(vMin,'(I4)') UFOLDDIMS(1)
   ELSEIF((UFOLDDIMS(1)/100).GE.1) THEN
    WRITE(vMin,'(I3)') UFOLDDIMS(1)
   ELSEIF((UFOLDDIMS(1)/10).GE.1) THEN
    WRITE(vMin,'(I2)') UFOLDDIMS(1)
   ELSE
    WRITE(vMin,'(I1)') UFOLDDIMS(1)
   ENDIF
   IF((UFOLDDIMS(2)/1000000000).GE.1) THEN
    WRITE(vMAX,'(I10)') N1
   ELSEIF((UFOLDDIMS(2)/100000000).GE.1) THEN
    WRITE(vMAX,'(I9)') N1
   ELSEIF((UFOLDDIMS(2)/10000000).GE.1) THEN
    WRITE(vMin,'(I8)') UFOLDDIMS(1)
   ELSEIF((UFOLDDIMS(2)/1000000).GE.1) THEN
    WRITE(vMin,'(I7)') UFOLDDIMS(1)
   ELSEIF((UFOLDDIMS(2)/100000).GE.1) THEN
    WRITE(vMin,'(I6)') UFOLDDIMS(1)
   ELSEIF((UFOLDDIMS(2)/10000).GE.1) THEN
    WRITE(vMAX,'(I5)') UFOLDDIMS(2)
   ELSEIF((UFOLDDIMS(2)/1000).GE.1) THEN
    WRITE(vMAX,'(I4)') UFOLDDIMS(2)
   ELSEIF((UFOLDDIMS(2)/100).GE.1) THEN
    WRITE(vMAX,'(I3)') UFOLDDIMS(2)
   ELSEIF((UFOLDDIMS(2)/10).GE.1) THEN
    WRITE(vMAX,'(I2)') UFOLDDIMS(2)
   ELSE
    WRITE(vMAX,'(I1)') UFOLDDIMS(2)
   ENDIF
   WRITE(vAVG,'(I1)') Dtype

  !TEMPTEXT1="a=readufdatafile('./"
  !TEMPTEXT2=oldName(1:FNL-8)//"/"//oldName(1:FNL)//"',"
   TEMPTEXT1="a=readufdatafile('"
   TEMPTEXT2=oldName(1:FNL)//"',"
   ...
   IF(NDIMS.EQ.3) THEN
    IF((OLDDIMS(1)/1000000000).GE.1) THEN
     WRITE(vMin,'(I10)') OLDDIMS(1)
    ELSEIF((OLDDIMS(1)/100000000).GE.1) THEN
     WRITE(vMin,'(I9)') OLDDIMS(1)
    ELSEIF((OLDDIMS(1)/10000000).GE.1) THEN
     WRITE(vMin,'(I8)') OLDDIMS(1)
    ELSEIF((OLDDIMS(1)/1000000).GE.1) THEN
     WRITE(vMin,'(I7)') OLDDIMS(1)
    ELSEIF((OLDDIMS(1)/100000).GE.1) THEN
     WRITE(vMin,'(I6)') OLDDIMS(1)
    ELSEIF((OLDDIMS(1)/10000).GE.1) THEN
     WRITE(vMin,'(I5)') OLDDIMS(1)
    ELSEIF((OLDDIMS(1)/1000).GE.1) THEN
     WRITE(vMin,'(I4)') OLDDIMS(1)
    ELSEIF((OLDDIMS(1)/100).GE.1) THEN
     WRITE(vMin,'(I3)') OLDDIMS(1)
    ELSEIF((OLDDIMS(1)/10).GE.1) THEN
     WRITE(vMin,'(I2)') OLDDIMS(1)
    ELSE
     WRITE(vMin,'(I1)') OLDDIMS(1)
    ENDIF
    IF((OLDDIMS(2)/1000000000).GE.1) THEN
     WRITE(vMAX,'(I10)') OLDDIMS(2)
    ELSEIF((OLDDIMS(2)/100000000).GE.1) THEN
     WRITE(vMAX,'(I9)') OLDDIMS(2)
    ELSEIF((OLDDIMS(2)/10000000).GE.1) THEN
     WRITE(vMAX,'(I8)') OLDDIMS(2)
    ELSEIF((OLDDIMS(2)/1000000).GE.1) THEN
     WRITE(vMAX,'(I7)') OLDDIMS(2)
    ELSEIF((OLDDIMS(2)/100000).GE.1) THEN
     WRITE(vMAX,'(I6)') OLDDIMS(2)
    ELSEIF((OLDDIMS(2)/10000).GE.1) THEN
     WRITE(vMAX,'(I5)') OLDDIMS(2)
    ELSEIF((OLDDIMS(2)/1000).GE.1) THEN
     WRITE(vMAX,'(I4)') OLDDIMS(2)
    ELSEIF((OLDDIMS(2)/100).GE.1) THEN
     WRITE(vMAX,'(I3)') OLDDIMS(2)
    ELSEIF((OLDDIMS(2)/10).GE.1) THEN
     WRITE(vMAX,'(I2)') OLDDIMS(2)
    ELSE
     WRITE(vMAX,'(I1)') OLDDIMS(2)
    ENDIF
  ...
  TEMPTEXT1="";TEMPTEXT2="";TEMPTEXT3="";TEMPTEXT4="";TEMPTEXT5=""
    IF((NEWDIMS(1)/1000000000).GE.1) THEN
     WRITE(txDIM1,'(I10)') NEWDIMS(1)
    ELSEIF((NEWDIMS(1)/100000000).GE.1) THEN
     WRITE(txDIM1,'(I9)') NEWDIMS(1)
    ELSEIF((NEWDIMS(1)/10000000).GE.1) THEN
     WRITE(txDIM1,'(I8)') NEWDIMS(1)
    ELSEIF((NEWDIMS(1)/1000000).GE.1) THEN
     WRITE(txDIM1,'(I7)') NEWDIMS(1)
    ELSEIF((NEWDIMS(1)/100000).GE.1) THEN
     WRITE(txDIM1,'(I6)') NEWDIMS(1)
    ELSEIF((NEWDIMS(1)/10000).GE.1) THEN
     WRITE(txDIM1,'(I5)') NEWDIMS(1)
    ELSEIF((NEWDIMS(1)/1000).GE.1) THEN
     WRITE(txDIM1,'(I4)') NEWDIMS(1)
    ELSEIF((NEWDIMS(1)/100).GE.1) THEN
     WRITE(txDIM1,'(I3)') NEWDIMS(1)
    ELSEIF((NEWDIMS(1)/10).GE.1) THEN
     WRITE(txDIM1,'(I2)') NEWDIMS(1)
    ELSE
     WRITE(txDIM1,'(I1)') NEWDIMS(1)
    ENDIF
    IF((NEWDIMS(2)/1000000000).GE.1) THEN
     WRITE(txDIM2,'(I10)') NEWDIMS(2)
    ELSEIF((NEWDIMS(2)/100000000).GE.1) THEN
     WRITE(txDIM2,'(I9)') NEWDIMS(2)
    ELSEIF((NEWDIMS(2)/10000000).GE.1) THEN
     WRITE(txDIM2,'(I8)') NEWDIMS(2)
    ELSEIF((NEWDIMS(2)/1000000).GE.1) THEN
     WRITE(txDIM2,'(I7)') NEWDIMS(2)
    ELSEIF((NEWDIMS(2)/100000).GE.1) THEN
     WRITE(txDIM2,'(I6)') NEWDIMS(2)
    ELSEIF((NEWDIMS(2)/10000).GE.1) THEN
     WRITE(txDIM2,'(I5)') NEWDIMS(2)
    ELSEIF((NEWDIMS(2)/1000).GE.1) THEN
     WRITE(txDIM2,'(I4)') NEWDIMS(2)
    ELSEIF((NEWDIMS(2)/100).GE.1) THEN
     WRITE(txDIM2,'(I3)') NEWDIMS(2)
    ELSEIF((NEWDIMS(2)/10).GE.1) THEN
     WRITE(txDIM2,'(I2)') NEWDIMS(2)
    ELSE
     WRITE(txDIM2,'(I1)') NEWDIMS(2)
    ENDIF


  TEMPTEXT="f = interpolate.interp2d(x,y,z,kind='"//TRIM(interpType)//"')"
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT='INDX=shape(x)' 
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT='xnew = np.linspace(x[0][0],x[INDX[0]-1][INDX[1]-1],'//TRIM(txDIM1)//')'
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT='ynew = np.linspace(y[0][0],y[INDX[0]-1][INDX[1]-1],'//TRIM(txDIM2)//')'
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT='znew2d = f(xnew,ynew)'
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT='[xnew2d,ynew2d]=meshgrid(xnew,ynew)'
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT='INDX=shape(xnew2d)' 
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT="xnew2d=transpose(xnew2d)"
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT="ynew2d=transpose(ynew2d)"
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT="znew2d=transpose(znew2d)"
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT='INDX=shape(xnew2d)' 
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT=''
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT="fw=open('ex"//TRIM(oldName)//"','w')"
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT="for iD1 in range(INDX[0]):"
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT="    for iD2 in range(INDX[1]):"
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT="        fw.write(('%.20f\\n')%(xnew2d[iD1][iD2]))"
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT="        fw.write(('%.20f\\n')%(ynew2d[iD1][iD2]))"
  WRITE(101,TRIM(LSTRFMT(LEN(TRIM(TEMPTEXT))))) TRIM(TEMPTEXT)
  TEMPTEXT="        fw.write(('%.20f\\n')%(znew2d[iD1][iD2]))"
...
! YOU CAN GO OVER THE CODE TO CHECK HOW CAN YOU USE IT.
 ...
 FUNCTION LSTRFMT(NUM)
  IMPLICIT NONE

  CHARACTER*3::NUMCHAR
  CHARACTER*6::LSTRFMT
  INTEGER::NUM,div100,mod100,div10,div1
  ...
  LSTRFMT='(A'//TRIM(NUMCHAR)//')'

 END FUNCTION LSTRFMT


!**********************************************
!*************** AVG VALUE *******************
!**********************************************
 FUNCTION R8AVG(inMAT)
  IMPLICIT NONE
  REAL(8) :: R8AVG,inMAT(:)

  R8AVG=SUM(inMAT)/SIZE(inMAT)
  RETURN
 END FUNCTION R8AVG

 FUNCTION R4AVG(inMAT)
  IMPLICIT NONE
  REAL(4) :: R4AVG,inMAT(:)

  R4AVG=SUM(inMAT)/SIZE(inMAT)
  RETURN
 END FUNCTION R4AVG

 FUNCTION I8AVG(inMAT)
  IMPLICIT NONE
  INTEGER(8) :: I8AVG,inMAT(:)

  I8AVG=SUM(inMAT)/SIZE(inMAT)
  RETURN
 END FUNCTION I8AVG

 FUNCTION I4AVG(inMAT)
  IMPLICIT NONE
  INTEGER(4) :: I4AVG,inMAT(:)

  I4AVG=SUM(inMAT)/SIZE(inMAT)
  RETURN
 END FUNCTION I4AVG
...
!**********************************************
 FUNCTION R8STDV(inMAT)
  ...
  DO iCONTR=1,SIZE(inMAT)
   tmp=tmp+(inMAT(iCONTR)-arrayAVG)**2
  END DO
  R8STDV=SQRT(tmp/SIZE(inMAT))
  RETURN
 END FUNCTION R8STDV

 FUNCTION R4STDV(inMAT)
  IMPLICIT NONE
  REAL(4)    :: R4STDV,inMAT(:)
  REAL(4)    :: tmp,arrayAVG
  INTEGER(4) :: iCONTR
  ...
  DO iCONTR=1,SIZE(inMAT)
   tmp=tmp+(inMAT(iCONTR)-arrayAVG)**2
  END DO
  R4STDV=SQRT(tmp/SIZE(inMAT))
  RETURN
 END FUNCTION R4STDV

 FUNCTION I8STDV(inMAT)
  IMPLICIT NONE
  INTEGER(8) :: inMAT(:)
  INTEGER(4) :: iCONTR
  REAL(8)    :: I8STDV
  ...
  DO iCONTR=1,SIZE(inMAT)
   tmp=tmp+(inMAT(iCONTR)-arrayAVG)**2
  END DO
  I8STDV=SQRT(tmp/SIZE(inMAT))
  RETURN
 END FUNCTION I8STDV

 FUNCTION I4STDV(inMAT)
  IMPLICIT NONE
  INTEGER(4) :: inMAT(:)
  INTEGER(4) :: iCONTR
  REAL(4)    :: I4STDV
  ...
  DO iCONTR=1,SIZE(inMAT)
   tmp=tmp+(inMAT(iCONTR)-arrayAVG)**2
  END DO
  I4STDV=SQRT(tmp/SIZE(inMAT))
  RETURN
 END FUNCTION I4STDV


END MODULE INTERPSUBS
